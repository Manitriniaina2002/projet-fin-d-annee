...
// Utiliser WiFiClient standard pour le port non-sécurisé
...
void setup()
{
  ...
  // Pas de certificat pour connexion non-sécurisée
...
void loop()
{
  ...
  if (millis() - last > 5000)
  {
    ...
    // JSON MQTT - Format compatible avec le backend
    char payload[256];
    snprintf(payload, sizeof(payload),
             "{\"sensors\":{\"temperature\":%.2f,\"humidity\":%.2f,\"distance\":%.2f,\"motion\":false},\"net\":{\"tx_bytes\":0,\"rx_bytes\":0,\"connections\":0}}",
             temp, hum, dist);

    // Topic format: devices/{device_id}/telemetry
    client.publish("devices/ESP32-001/telemetry", payload);
